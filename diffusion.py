...
def CrossAttention(heads, width):
    def apply(x_in, y_in, use_label=None):
        ...
        x = layers.MultiHeadAttention(num_heads=heads, key_dim=64*heads, kernel_initializer=kernel_init(1.0))(query=x, key=y, value=y) # , value_dim=width
...
def ResidualBlock(width, k=3, groups=16, use_t=True):
    def apply(x, t):
        ...
        x = layers.GroupNormalization(epsilon=1e-6, groups=groups)(x)
        x = layers.Activation('swish')(x)
        #x = layers.SeparableConv2D(width*2, kernel_size=k, padding="same", kernel_initializer=kernel_init(1.0))(x)
        ...
        x = layers.Activation('swish')(x)
        x = SqueezeExcite(x)
        #x = layers.SeparableConv2D(width, kernel_size=3, padding="same")(x)
        x = layers.Conv2D(width*1, kernel_size=3, padding="same", kernel_initializer=kernel_init(1.0))(x)
...
def get_network(image_size, widths, block_depth):
    ...
    spatial_position = PositionalEncoding2D(noisy_images.shape[2], noisy_images.shape[1], widths[0]*1)(noisy_images)
    
    noise_variances = layers.Input(shape=(1, 1, 1))
    temb = TimeEmbedding(dim=widths[0] * 4)(keras.backend.flatten(noise_variances))
    temb = TimeMLP(units=widths[0])(temb)
    ...
    x = MiddleBlock(widths[-1], block_depth)([x, caption_embed, temb, use_label])
    x = MiddleBlock(widths[-1], block_depth)([x, caption_embed, temb, use_label])
    
    for width in reversed(widths[:-1]):
        x, skips = UpBlock(width, 3, block_depth)([x, skips, caption_embed, temb, use_label])
    
    #x = tfa.layers.GroupNormalization(epsilon=1e-6, groups=16)(x)
    x = layers.Activation('swish')(x)
    x = layers.Conv2D(3, kernel_size=1, kernel_initializer=kernel_init(0.0))(x)
    return keras.Model([noisy_images, noise_variances, image_caption, use_label], x, name="residual_unet")
...
class DiffusionModel(keras.Model):
    ...
    def compile(self, **kwargs):
        ...
        #self.clean_validity_loss_tracker = keras.metrics.Mean(name="v_loss")
        #self.noisy_validity_loss_tracker = keras.metrics.Mean(name="n_v_loss")
        #self.clean_class_acc_tracker = keras.metrics.Mean(name="acc")
        #self.noisy_class_acc_tracker = keras.metrics.Mean(name="n_acc")
    ...
    def denoise(self, noisy_images, noise_rates, image_labels, signal_rates, training, use_label=True):
        ...
        if use_label:
            ...
            ul = tf.zeros((1, 1, 1))
        pred_noises = network([noisy_images, noise_rates**2, image_labels, ul], training=training)
        #else:
    ...
    def reverse_diffusion(self, initial_noise, diffusion_steps, image_labels):
        ...
        for step in range(diffusion_steps):
            ...
            pred_noises, _ = self.denoise(noisy_images, noise_rates, image_labels, signal_rates, training=False)
            cf_pred_noises, _ = self.denoise(noisy_images, noise_rates, image_labels, signal_rates, training=False, use_label=False)
    ...
    def train_step(self, data_in):
        ...
        with tf.GradientTape() as gen_tape:
            ...
            pred_noises, pred_images = self.denoise(noisy_images, noise_rates, caption_embed, signal_rates, training=True, use_label=np.random.random() > 0.1)
    ...
    def test_step(self, data_in):
        ...
        pred_noises, pred_images = self.denoise(noisy_images, noise_rates, caption_embed, signal_rates, training=False)
        ...
        generated_images = self.generate(num_images=batch_size, labels=caption_embed, diffusion_steps=kid_diffusion_steps)
    ...
    def plot_images(self, epoch=None, logs=None, num_rows=3, num_cols=3, steps=plot_diffusion_steps):
        ...
        generated_images = self.generate(num_images=num_rows * num_cols, labels=caption_embed, diffusion_steps=steps)
        ...
        #plt.show()
        #if epoch % 5 == 0:
        plt.savefig(f"images/diffusion_{epoch}.png")
        plt.close()
...
if __name__ == '__main__':   
    ...
    checkpoint_path = "checkpoints3/diffusion_model"
    ...
    reduce = keras.callbacks.ReduceLROnPlateau(monitor='val_kid', mode='min', factor=0.5, patience=10, verbose=1, min_lr=1e-5)
    ...
    #print(model.normalizer.mean, model.normalizer.variance)

    # run training and plot generated images periodically
    model.load_weights("checkpoints3/diffusion_model")
